...
|---|---|---|
| 2.2 | Junio 2026 | Borrador para validación |

**Cambios respecto a v2.1:** se eliminan las salidas de post de LinkedIn y artículo WordPress. El contenido editorial de marca (publicaciones, artículos) se produce fuera de ConvoKit, en el flujo editorial propio de Innóvate 4.0, para no arriesgar invención de datos o experiencia en piezas firmadas públicamente. ConvoKit queda con cinco salidas, todas técnicas y de bajo riesgo de invención. La landing se mantiene con regla estricta de no inventar nada que no esté en las bases. Salidas renumeradas.

---

## 1. Contexto y problema que resuelve

Innóvate 4.0 produce, por cada convocatoria relevante que abre, un conjunto fijo de documentos de manera manual: guía interna para el consultor, ficha comercial para el cliente de Ruta i40, landing page para innovate40.es, set de prompts para redactar la memoria de solicitud y lista de documentación requerida al cliente.
...
Construir una aplicación web interna (uso exclusivo del equipo de Innóvate 4.0) que, a partir de documentos oficiales de convocatoria en múltiples formatos, genere los cinco tipos de entregables definidos mediante llamadas a la API de Claude, con histórico persistente de convocatorias procesadas.
...
- Generación de los cinco entregables mediante Claude API.
...
- Contenido editorial de marca (posts de LinkedIn, artículos de blog/WordPress). Se produce fuera de ConvoKit, en el flujo editorial propio de Innóvate 4.0, donde se controla la voz y se evita la invención de datos o experiencia en piezas firmadas públicamente. La guía del consultor (salida 1) aporta los datos verificados que luego alimentan ese contenido editorial.
...
## 4. Menú de salidas (cinco entregables)
...
|---|---|---|---|
| 1 | Guía interna del consultor | .md / texto | Baremo, criterios, perfil ideal del beneficiario, a quién comercializar y plan de trabajo. Destinatario: consultor de Innóvate 4.0. Es además la fuente de datos verificados para el contenido editorial que se produzca fuera de ConvoKit. |
...
| 3 | Landing page | .md | Estructura completa de la landing en .md con jerarquía clara para Claude design. Especifica secciones, jerarquía y CTAs. Regla estricta: no inventar nada que no esté en las bases. |
| 4 | Set de prompts para la memoria | .md + .json | Un prompt por sección de la memoria. Cada prompt pide al consultor la documentación necesaria antes de redactar. Nivel de detalle suficiente para que el consultor no necesite redactar nada de cero. Exporta también JSON estructurado (perfil de convocatoria). |
| 5 | Lista de documentación + correo al cliente | .md + .json + texto | Lista estructurada de documentos exigidos por la convocatoria, más correo tipo listo para enviar al cliente solicitándolos. Exporta también JSON estructurado (árbol de documentos). |
...
| /convocatorias/{id}/generate | POST | Genera uno o varios entregables (output_types: lista de 1-5). Guarda en SQLite. Devuelve textos generados. |
...
- Selector de entregables: checkboxes para cada una de las cinco salidas.
...
- Botón "Descargar" en cada panel: .md para las salidas textuales, .txt para el correo al cliente (salida 5).
- Botón "Regenerar" en cada panel: regenera solo ese entregable sin tocar los demás.
- Las salidas 4 y 5 tienen dos botones de descarga: ".md" (legible) y ".json" (estructurado).
...
- max_tokens: 4096 para salidas 1, 2, 3 y 5. max_tokens: 8192 para salidas 1 y 4 (guía del consultor y set de prompts, que son las más extensas). Concretamente: salida 1 (guía) 8192, salida 4 (prompts) 8192, resto 4096.
...
- REGLA ESTRICTA DE NO INVENCIÓN: todo dato (importes, porcentajes, plazos, requisitos, sectores) debe salir literalmente de los documentos de la convocatoria. Prohibido inventar cifras, estadísticas, casos o afirmaciones que no estén en las bases. Es contenido público de marca: si un dato no está en el documento, no se incluye.
...
Las salidas 4 y 5 generan, además del markdown legible, una versión en JSON estructurado. El JSON contiene exactamente la misma información, etiquetada por campos para que una segunda aplicación pueda leerla sin intervención humana.

> Contexto: Innóvate 4.0 va a construir una segunda aplicación (App de Memorias) que redacta memorias técnicas y rellena cuentas justificativas a partir de los datos reales de cada empresa. Esa app necesita, por cada convocatoria, un perfil de convocatoria (secciones, baremo, inputs) y un árbol de documentos. El JSON de las salidas 4 y 5 de ConvoKit es precisamente ese input. La conexión en el MVP es manual: se descarga el JSON de ConvoKit y se carga en la App de Memorias.

### 12.1 JSON de la salida 4 — perfil de convocatoria
...
### 12.2 JSON de la salida 5 — árbol de documentos
...
- Las salidas 4 y 5 tienen dos botones de descarga: "Descargar .md" (legible) y "Descargar .json" (estructurado).
...
- Las salidas 4 y 5 exportan JSON estructurado precisamente para alimentar la App de Memorias: la salida 4 produce el perfil de convocatoria (secciones, baremo, inputs); la salida 5 produce el árbol de documentos.
...
- /backend/exporters.py con la lógica de exportación de las salidas 4 y 5 a JSON estructurado, según el esquema de la sección 12.
...
- Paso 8: Backend — implementar las salidas 2 a 5, una a una, validando cada prompt con INPYME 2026.
- Paso 8 bis: Backend — exporters.py: exportación de las salidas 4 y 5 a JSON estructurado según el esquema de la sección 12.
...
| 3 | Las cinco salidas se generan sin error para INPYME 2026. | Manual: generar los cinco entregables y verificar coherencia con las bases. |
...
| 8 | El set de prompts (salida 4) cubre todos los apartados de la memoria de INPYME 2026 y cada prompt especifica los documentos que el consultor debe aportar. | Manual: contrastar los prompts con el índice de la plantilla de memoria de INPYME 2026. |
| 9 | El correo tipo (salida 5) es enviable directamente con mínima edición (solo rellenar campos entre corchetes). | Manual: leer el correo y evaluar si es accionable sin reescritura. |
| 10 | Las salidas 4 y 5 se descargan también en JSON con el esquema de campos de la sección 12. | Manual: descargar ambos JSON de INPYME 2026 y verificar que tienen los campos definidos y son JSON válido. |
...
- Salidas 2 y 3 (ficha comercial y landing) en formato .md para Claude design, no HTML autónomo.
- Extracción de texto: librerías locales, sin consumo de tokens.
- Salidas 4 y 5 exportan también JSON estructurado para alimentar la App de Memorias (segunda app). Integración manual en el MVP: descarga e importación de fichero.
- Eliminadas las salidas de LinkedIn y WordPress: el contenido editorial se produce fuera de ConvoKit.
...
*Innóvate 4.0 · ConvoKit PRD v2.2 · Junio 2026*
