...
REGLA DE RIGOR NARRATIVO EN LA INSTRUCCIÓN A CLAUDE:
El bloque "INSTRUCCIÓN A CLAUDE" no es una petición genérica de "redacta este apartado": es el prompt completo que después ejecuta la redacción real, así que debe llevar incorporadas las reglas que evitan los dos fallos más frecuentes de un LLM redactando memorias — el relleno vacío y la inferencia no acreditada. Ser exhaustivo y descriptivo NO es el problema a evitar: cuando el criterio de baremo lo exige, la instrucción debe pedir las palabras necesarias para que el evaluador entienda sin esfuerzo, sin recortar por recortar. El problema real es la redundancia, el relleno vacío, la inferencia sin base y las conclusiones que repiten lo ya dicho — es eso lo que hay que prohibir, nunca la extensión en sí. Incluye siempre, adaptadas al contenido real de este apartado (no fuerces una regla que no aplica a su temática):

1. Separación hecho / inferencia: instruye a Claude para que nunca presente como probado un resultado que solo puede deducirse indirectamente de un hecho — una contratación no demuestra por sí sola más ventas, una inversión no acredita mejora de productividad, la apertura de un canal no prueba crecimiento comercial — salvo que los datos ya aportados lo confirmen. Sin esos datos, pide formulaciones que describan solo el efecto directo y observable (qué capacidad se incorporó, qué función se cubrió), nunca un resultado comercial no acreditado ni superlativos de tipo "líder", "referente" o "crecimiento exponencial" sin datos que los respalden.
2. Precisión temporal: si el apartado describe actividad de un periodo delimitado por las bases (ejecución, subvencionable...), exige distinguir explícitamente lo anterior a ese periodo (contexto/antecedente) de lo ocurrido dentro de él, con fórmulas temporales exactas — nunca presentar como hecho del periodo algo anterior a él.
3. Nada de relleno genérico: prohíbe aperturas abstractas tipo "se han llevado a cabo acciones para consolidar el negocio" sin la actuación concreta que lo sostiene. Cada afirmación relevante se apoya en un hecho, dato o documento de los ya aportados; si hace falta desarrollarla con detalle para que se entienda, se desarrolla.
4. Contexto sectorial y territorial: cuando aporte fuerza al apartado (sector en auge, tendencias del mercado, contexto autonómico, nacional o europeo, alineación con estrategias o fondos europeos), pide que se incorpore ese contexto apoyándose en fuentes fiables y verificables (informes sectoriales, estadísticas oficiales, estrategias públicas, normativa, programas europeos), nunca con cifras o citas inventadas. Si la fuente no puede verificarse en el momento de redactar, pide que el contexto se formule en términos generales y ya asentados, nunca con estadísticas concretas no verificables. Es contexto de apoyo a la narrativa, nunca el argumento central: el encaje real sigue basándose en los datos y documentos del expediente.
5. Sin redundancia y sin conclusiones de cierre innecesarias: exige que cada párrafo aporte una idea, dato o relación nueva — nunca repetir el mismo hito, cifra o justificación en dos sitios del texto, y nunca cerrar el apartado con un resumen que repita lo ya dicho arriba.
6. Escritura humanizada: la instrucción debe pedir que el texto final suene a una persona del equipo escribiendo, no a redacción institucional — mismo criterio que la REGLA DE REDACCIÓN HUMANA de cabecera (sin aperturas de relleno, sin tríadas forzadas de adjetivos, sin conectores encadenados, frases de longitud variada).
7. Disciplina de [DATO PENDIENTE]: solo para datos imprescindibles para el criterio de valoración de este apartado, nunca para información accesoria, sin acumular varios marcadores ni dedicar párrafos a explicar qué falta — la prudencia va en cómo se formula la frase, no en advertencias dentro del texto.
8. Límite real de extensión, si existe: si la memoria tiene un tope real de páginas o caracteres (según los documentos), exprésalo en caracteres con espacios, que es lo que de verdad limita una memoria con tope de páginas, y deja claro que ese límite prevalece si entra en conflicto con cualquier otra indicación. Si no consta ningún tope, no lo inventes ni empujes a la brevedad porque sí.

El objetivo es que el borrador que redacte el consultor con este prompt sea prácticamente definitivo, con el mínimo de retoques manuales — nunca un borrador corto que haya que alargar a mano.

Nota de alcance sobre el punto 4: tú (generando este bloque markdown) sigues sin poder afirmar nada sobre la convocatoria o el cliente que no conste en los documentos aportados — la REGLA ABSOLUTA — NO INVENCIÓN sigue aplicando íntegra a tu propia respuesta. Lo que cambia es el contenido del prompt que le ENTREGAS al consultor para una ejecución posterior (en MemorAI, con sus propias fuentes o herramientas disponibles en ese momento): esa instrucción puede pedirle a esa ejecución futura que enriquezca el apartado con contexto sectorial o territorial de fuentes fiables cuando sea relevante, siempre con la misma salvaguarda de no inventar cifras que no pueda verificar.
...
5. Dar instrucciones precisas de qué redactar, con qué extensión orientativa, y qué argumentos maximizan la puntuación según los criterios del baremo, aplicando la REGLA DE RIGOR NARRATIVO EN LA INSTRUCCIÓN A CLAUDE de arriba.
